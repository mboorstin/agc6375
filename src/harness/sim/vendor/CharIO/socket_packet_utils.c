...
int getPortNumber(const char * name, unsigned int dflt_port)
{
  ...
  if (s != NULL) port = atoi(s);
    ...
    printf("[Harness] %s environment variable not defined, using default port %d instead\n", env_var_name, dflt_port);
    port = (int) dflt_port;
  }
...
inline void acceptConnection(serv_socket_state_t * s)
{
  ...
  if (s->conn != -1) {
    printf("[Harness] %s socket got a connection\n", s->name);
...
unsigned long long serv_socket_create(const char * name, unsigned int dflt_port)
{
  ...
  s->sock = -1;
  s->conn = -1;
  printf("[Harness] allocated socket for %s\n", s->name);
...
extern inline void serv_socket_init(unsigned long long ptr)
{
  ...
  printf("[Harness] %s socket listening on port %d\n", s->name, s->port);
}
